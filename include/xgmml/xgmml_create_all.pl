...
sub calculate_edges{
	my $max_edges = "10000000";
        my @sequences = split "\n", `LC_ALL=C fgrep ">" $fasta | cut -f2 -d'>'`;
	my $edge_count = 0;
	print "Found " . $#sequences .  " sequence_count for $fasta\n";
	foreach my $sequence(@sequences){
		chomp(my $count = `LC_ALL=C fgrep -c "$sequence" $blast`);
		$edge_count+=$count;
		print "Found $edge_count \n";
