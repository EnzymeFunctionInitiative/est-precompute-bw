...
if(!-s $blast){
	$blast = "$dir/demux/$unit_name.1out.demultiplexed";
}
die "$fasta not found\n" unless (-s $fasta);
die "$blast not found\n" unless (-s $blast);
...
if($count == 0){
	...
	die "Failure: Results of call = $call" unless($call==0);	

}
else{
	print "No need to run `Rgraphs.pl`, we have the outputs from it already. We only need to run the R scripts ($count)\n";
}
...
if(-s $FIRST){
	...
	print "Couldn't open $FIRST (ls $rdata_dir/perid*), skipping quartiles \n";
}
...
if(-s $LAST){
	...
	print "Couldn't open $LAST (ls $rdata_dir/perid*), skipping quartiles \n";
}
